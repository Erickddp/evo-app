...
export const BankReconcilerTool: React.FC = () => {
    ...
    const [importStats, setImportStats] = useState<{ new: number; updated: number } | null>(null);
    ...
    const handleImport = async () => {
        ...
        try {
            const stats = await ingestBankMovements(result.movements);
            ...
            {result && (
                <div className="w-full max-w-2xl animate-in slide-in-from-bottom-4 duration-500">
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                        ...
                        {importStats ? (
                            <div className="p-8 bg-emerald-50 dark:bg-emerald-900/10 border-t border-emerald-100 dark:border-emerald-800">
                                <h4 className="text-lg font-semibold text-emerald-700 dark:text-emerald-400 mb-2">¡Importación Completada!</h4>
                                <div className="flex justify-center gap-6 text-sm text-gray-600 dark:text-gray-300">
                                    <span className="flex items-center gap-2">
                                        <span className="w-2 h-2 rounded-full bg-emerald-500" />
                                        Nuevos: <b>{importStats.new}</b>
                                    </span>
                                    <span className="flex items-center gap-2">
                                        <span className="w-2 h-2 rounded-full bg-blue-500" />
                                        Actualizados: <b>{importStats.updated}</b>
                                    </span>
                                </div>
                                <button
                                    onClick={() => { setResult(null); setFile(null); setImportStats(null); }}
                                    className="mt-6 text-sm text-emerald-600 dark:text-emerald-400 hover:text-emerald-800 dark:hover:text-emerald-200 underline"
                                >
                            ...
                            <div className="p-8 bg-gray-50 dark:bg-gray-800/50 flex flex-col items-center gap-4">
                                {error && (
                                    <div className="w-full mb-2 p-3 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-300 rounded-lg text-sm text-center">
                                        {error}
                                    </div>
                                )}
                                ...
                                <button
                                    ...
                                    className="w-full md:w-auto px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-lg shadow-lg shadow-blue-500/30 transition-all transform hover:-translate-y-1 flex items-center justify-center gap-3"
                                >
                                    {isImporting ? <Loader2 className="w-6 h-6 animate-spin" /> : <Save className="w-6 h-6" />}
                                    {isImporting ? 'Guardando...' : 'Guardar en Sistema'}
                                </button>

                                {/* Secondary Action: CSV Download */}
                                <button
